    ...
    if (instructionsElement) {
      ...
      if (videoCount >= 3) {
        ...
        adskipUtils.logDebug(
          "在options页面显示使用说明，当前处理视频数:",
          videoCount,
        );
      } else {
        instructionsElement.style.display = "none";
        adskipUtils.logDebug(
          "在options页面隐藏使用说明，当前处理视频数:",
          videoCount,
        );
      }
    }
...
function renderWhitelist() {
  ...
  // 更新计数
  const enabledCount = whitelistData.filter(
  ...
  if (whitelistData.length === 0) {
    container.innerHTML =
      '<div class="whitelist-empty">白名单为空，您可以在视频页面将UP主添加到白名单</div>';
    return;
  }
...
document.addEventListener("DOMContentLoaded", function () {
      ...
      if (isAdmin) {
        ...
        if (confirm("确定要退出管理员登录吗？")) {
          chrome.storage.local.set(
            { [adskipStorage.KEYS.ADMIN_AUTH]: false },
            function () {
  ...
  const adminUpdateAdsSecretInput = document.getElementById(
    "admin-update-ads-secret",
  );
  const saveAdminUpdateAdsSecretButton = document.getElementById(
    "save-admin-update-ads-secret",
  );
  if (adminUpdateAdsSecretInput && saveAdminUpdateAdsSecretButton) {
    ...
    saveAdminUpdateAdsSecretButton.addEventListener("click", async function () {
      await adskipStorage.setAdminUpdateAdsSecret(
        adminUpdateAdsSecretInput.value.trim(),
      );
    ...
    adskipStorage.getSearchPrecheck().then(function (currentSearchPrecheck) {
      ...
      if (currentSearchPrecheck !== newSearchPrecheck) {
        adskipStorage.setSearchPrecheck(newSearchPrecheck).then(function () {
          showStatus(
            newSearchPrecheck ? "已启用搜索页预检功能" : "已禁用搜索页预检功能",
          );
        });
      }
    });
  ...
  const subtitleTimelineDefaultCollapsedToggle = document.getElementById(
    "subtitle-timeline-default-collapsed",
  );
  subtitleTimelineDefaultCollapsedToggle.addEventListener(
    "change",
    function () {
      adskipStorage.setSubtitleTimelineDefaultCollapsed(this.checked).then(
        function () {
          showStatus(
            this.checked
              ? "已设置字幕动态轴默认收起"
  ...
  // 导入白名单按钮
  document
    .getElementById("whitelist-import")
  ...
  // 导出白名单按钮
  document
    .getElementById("whitelist-export")
    .addEventListener("click", function () {
      const textarea = document.getElementById("whitelist-textarea");
  ...
  document
    .getElementById("enable-custom-server")
    .addEventListener("change", function () {
      toggleCustomServer();
    });

  document
    .getElementById("reset-custom-server")
    .addEventListener("click", function () {
      resetCustomServer();
    });

  document
    .getElementById("custom-server-url")
...
function loadSettings() {
  ...
  Promise.all([
    ...
    if (subtitleTimelineDefaultCollapsedToggle) {
      subtitleTimelineDefaultCollapsedToggle.checked =
        subtitleTimelineCollapsed;
    }
  });
}
...
chrome.storage.onChanged.addListener(function (changes, namespace) {
  ...
  if (changes[adskipStorage.KEYS.DEBUG_MODE] !== undefined) {
    ...
    if (debugModeToggle) {
      debugModeToggle.checked =
        changes[adskipStorage.KEYS.DEBUG_MODE].newValue || false;
    }
  }
  ...
  if (changes[adskipStorage.KEYS.SKIP_OWN_VIDEOS] !== undefined) {
    const skipOwnVideosToggle = document.getElementById("skip-own-videos");
    if (skipOwnVideosToggle) {
      skipOwnVideosToggle.checked =
        changes[adskipStorage.KEYS.SKIP_OWN_VIDEOS].newValue !== false;
    }
  }
  ...
  if (changes[adskipStorage.KEYS.SEARCH_PRECHECK] !== undefined) {
    const searchPrecheckToggle = document.getElementById("search-precheck");
    if (searchPrecheckToggle) {
      searchPrecheckToggle.checked =
        changes[adskipStorage.KEYS.SEARCH_PRECHECK].newValue === true;
    }
  }

  if (
    changes[adskipStorage.KEYS.SUBTITLE_TIMELINE_DEFAULT_COLLAPSED] !==
    undefined
  ) {
    const subtitleTimelineDefaultCollapsedToggle = document.getElementById(
      "subtitle-timeline-default-collapsed",
...
function loadCustomServerSettings() {
  chrome.storage.sync.get(
    ["customServerUrl", "customServerEnabled"],
    function (result) {
...
function toggleCustomServer() {
  ...
  if (newEnabled) {
    ...
    if (!isValidServerUrl(url)) {
      ...
      showCustomServerStatus(
        "无效的服务器地址。支持 https:// 或 http://localhost:3000",
        "error",
      );
      return;
    }
...
function resetCustomServer() {
  if (
    confirm(
      "确定要重置自定义服务器设置吗？这将清除保存的服务器地址并关闭自定义服务器功能。",
    )
  ) {
    ...
    chrome.storage.sync.get(
      ...
      function (result) {
        updateCustomServerStatus(
          result.customServerEnabled,
          result.customServerUrl,
        );
      },
    );
  }, 3000);
...
async function initSkipIntroOutroSettings() {
  ...
  const skipIntroDuration = document.getElementById(
    "skip-intro-duration-option",
  );
  const skipOutroEnabled = document.getElementById("skip-outro-enabled");
  const skipOutroDuration = document.getElementById(
    "skip-outro-duration-option",
  );

  if (!skipIntroEnabled) return; // 元素不存在则退出

  // 加载当前设置
  const [introEnabled, introDuration, outroEnabled, outroDuration] =
    await Promise.all([
  ...
  // 导入按钮
  document
    .getElementById("skipintro-import")
  ...
  // 导出按钮
  document
    .getElementById("skipintro-export")
    .addEventListener("click", async function () {
      const textarea = document.getElementById("skipintro-uploader-textarea");
      ...
      showStatus(
        `已导出 ${list.filter((item) => item.enabled !== false).length} 个UP主到文本框`,
      );
    });

  // 复制到剪贴板按钮
  document
    .getElementById("skipintro-copy")
    .addEventListener("click", function () {
      const textarea = document.getElementById("skipintro-uploader-textarea");
      const text = textarea.value.trim();
...
async function loadSkipIntroOutroUploaderList() {
  try {
    skipIntroOutroUploaderData =
      await adskipStorage.getSkipIntroOutroUploaderList();
    renderSkipIntroOutroUploaderList();
  } catch (error) {
...
function renderSkipIntroOutroUploaderList() {
  ...
  // 更新计数
  const enabledCount = skipIntroOutroUploaderData.filter(
    (item) => item.enabled !== false,
  ).length;
  ...
  if (skipIntroOutroUploaderData.length === 0) {
    container.innerHTML =
      '<div class="whitelist-empty">列表为空，您可以在视频页面的插件弹窗中将UP主添加到此列表</div>';
    return;
  }
  ...
  skipIntroOutroUploaderData.forEach(function (item, index) {
    ...
    const introDuration =
      item.introDuration !== undefined
        ? item.introDuration
        : globalDefaultIntroDuration;
    const outroDuration =
      item.outroDuration !== undefined
        ? item.outroDuration
        : globalDefaultOutroDuration;
...
chrome.storage.onChanged.addListener(function (changes, namespace) {
  ...
  if (changes[adskipStorage.KEYS.SKIP_INTRO_ENABLED] !== undefined) {
    ...
    if (skipIntroEnabled) {
      skipIntroEnabled.checked =
        changes[adskipStorage.KEYS.SKIP_INTRO_ENABLED].newValue === true;
    }
  }
  ...
  if (changes[adskipStorage.KEYS.SKIP_INTRO_DURATION] !== undefined) {
    const newVal =
      changes[adskipStorage.KEYS.SKIP_INTRO_DURATION].newValue || 0;
    const skipIntroDuration = document.getElementById(
      "skip-intro-duration-option",
    );
  ...
  if (changes[adskipStorage.KEYS.SKIP_OUTRO_ENABLED] !== undefined) {
    const skipOutroEnabled = document.getElementById("skip-outro-enabled");
    if (skipOutroEnabled) {
      skipOutroEnabled.checked =
        changes[adskipStorage.KEYS.SKIP_OUTRO_ENABLED].newValue === true;
    }
  }
  ...
  if (changes[adskipStorage.KEYS.SKIP_OUTRO_DURATION] !== undefined) {
    const newVal =
      changes[adskipStorage.KEYS.SKIP_OUTRO_DURATION].newValue || 0;
    const skipOutroDuration = document.getElementById(
      "skip-outro-duration-option",
    );
  ...
  // 监听UP主列表变化
  if (
    changes[adskipStorage.KEYS.SKIP_INTRO_OUTRO_UPLOADER_LIST] !== undefined
  ) {
    if (window.location.hash === "#skipintro") {
      loadSkipIntroOutroUploaderList();
    }
