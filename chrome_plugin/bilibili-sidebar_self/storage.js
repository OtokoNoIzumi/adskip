...
const STORAGE_KEYS = {
  ...
  READ_BVS: "adskip_read_bvs",
  TRANSCRIBED_SUBTITLE_PREFIX: "adskip_tsub_",
  ...
  SUBTITLE_TIMELINE_DEFAULT_COLLAPSED:
  ...
  WHITELIST_KEYS: ["adskip_uploader_whitelist"],
      ...
      chrome.storage.local.get(null, (allData) => {
        if (chrome.runtime.lastError) {
          adskipUtils.logDebug(
            `获取所有存储键失败: ${chrome.runtime.lastError.message}`,
          );
          resolve([]);
          return;
        }
        ...
        adskipUtils.logDebug(
          `找到 ${allKeys.length} 个存储键，其中 ${videoKeys.length} 个是视频数据键`,
        );

        if (videoKeys.length > 0) {
          adskipUtils.logDebug(
            `视频数据键示例: ${videoKeys.slice(0, 3).join(", ")}${videoKeys.length > 3 ? "..." : ""}`,
          );
        }
    ...
    return new Promise((resolve) => {
      chrome.storage.local.remove(videoKeys, () => {
        ...
        if (success) {
          ...
          adskipUtils.logDebug(
            `清除视频数据失败: ${chrome.runtime.lastError?.message || "未知错误"}`,
          );
    ...
    try {
      ...
      chrome.storage.local.get(storageKey, (result) => {
        ...
        try {
          ...
          const hasTimestamps =
            parsed.timestamps && Array.isArray(parsed.timestamps);
          adskipUtils.logDebug(
            `成功加载视频 ${videoId} 的广告时间段，包含 ${hasTimestamps ? parsed.timestamps.length : 0} 个时间段`,
          ...
          adskipUtils.logDebug(
            `解析视频 ${videoId} 数据时出错: ${parseError.message}`,
          );
          resolve([]);
        }
      });
...
async function loadAndValidateTimestamps(videoId, urlTimestamps = []) {
  ...
  adskipUtils.logDebug(
    `开始加载和验证广告时间戳 - 视频ID: ${videoId}, URL参数数量: ${urlTimestamps?.length || 0}`,
  );
  ...
  try {
    ...
    adskipUtils.logDebug(
      `当前视频已存储的时间戳数量: ${savedTimestamps.length}`,
    );

    // 2. 如果没有URL时间戳，直接返回存储的时间戳
    if (
      !urlTimestamps ||
      !Array.isArray(urlTimestamps) ||
      urlTimestamps.length === 0
    ) {
    ...
    const videoKeys = await getVideoDataKeys();
    // 过滤掉当前视频
    const otherVideoKeys = videoKeys.filter(
      (key) => key !== `${STORAGE_KEYS.VIDEO_PREFIX}${videoId}`,
    );

    adskipUtils.logDebug(
      `找到 ${otherVideoKeys.length} 个其他视频的数据用于污染检测`,
    );
    ...
    adskipUtils.logDebug(
      `检查URL参数 [${urlTimeString}] 是否与其他视频的广告时间戳匹配`,
    );
    ...
    for (const key of otherVideoKeys) {
      ...
      try {
        ...
        if (Array.isArray(timestamps) && timestamps.length > 0) {
          ...
          adskipUtils.logDebug(
            `比对视频 #${checkCount}: ${key.replace(STORAGE_KEYS.VIDEO_PREFIX, "")}, 时间戳字符串: ${savedTimeString}`,
          );
          ...
          if (urlTimeString === savedTimeString) {
            ...
            adskipUtils.logDebug(
              `URL参数污染检测: 视频 ${matchedVideoId} 的时间戳与URL参数相同，判定为视频切换造成的污染!`,
            );
            break;
          }
        } else {
          adskipUtils.logDebug(
            `视频 ${key.replace(STORAGE_KEYS.VIDEO_PREFIX, "")} 没有有效的时间戳数据`,
          );
        }
      } catch (e) {
...
function saveAdTimestampsForVideo(videoId, timestamps) {
  ...
  if (!timestamps || !Array.isArray(timestamps)) {
    adskipUtils.logDebug(
      `保存失败：时间戳无效或不是数组 (${typeof timestamps})`,
    );
    return Promise.resolve(false);
  }

  adskipUtils.logDebug(
    `准备保存 ${timestamps.length} 个广告时间段到视频 ${videoId}`,
  );
  ...
  return new Promise(async (resolve) => {
    try {
      ...
      chrome.storage.local.set(saveObj, () => {
        ...
        if (success) {
          adskipUtils.logDebug(
            `成功保存广告时间段：${timestamps.length} 条时间戳已保存到视频 ${videoId}`,
          );
        } else {
          adskipUtils.logDebug(
            `保存广告时间段失败: ${chrome.runtime.lastError?.message || "未知错误"}`,
          );
 ...
 * @param {string} [insightData.mainpoint] 核心观点
...
function saveVideoInsightData(videoId, insightData) {
  ...
  const reason =
    typeof insightData.mainpoint === "string"
      ? insightData.mainpoint.trim()
      : "";
  const points = Array.isArray(insightData.keyPoints)
  ...
  return new Promise((resolve) => {
    ...
    chrome.storage.local.set(saveObj, () => {
      ...
      if (success) {
        ...
        adskipUtils.logDebug(
          `[AdSkip存储] 保存洞察数据失败: ${chrome.runtime.lastError?.message}`,
        );
      }
      resolve(success);
    });
 ...
 * 加载视频的 AI 洞察数据
 * @param {string} videoId 视频ID
 * @returns {Promise<Object>} { mainpoint: string, keyPoints: Array }
 */
function loadVideoInsightData(videoId) {
  if (!videoId) {
    return Promise.resolve({ mainpoint: "", keyPoints: [] });
  }
  ...
  return new Promise((resolve) => {
    chrome.storage.local.get(key, (result) => {
      ...
      try {
        ...
        adskipUtils.logDebug(
          `[AdSkip存储] 解析视频 ${videoId} 洞察数据失败: ${e.message}`,
        );
        resolve({ mainpoint: "", keyPoints: [] });
      }
    });
...
async function loadAdSkipPercentage() {
  ...
  return new Promise((resolve) => {
    chrome.storage.local.get(STORAGE_KEYS.PERCENTAGE, (data) => {
      ...
      if (chrome.runtime.lastError) {
        adskipUtils.logDebug(
          `加载广告跳过百分比配置失败: ${chrome.runtime.lastError.message}，使用默认值 1%`,
        );
        resolve(1);
        return;
      }
  ...
  return new Promise((resolve) => {
    ...
    chrome.storage.local.set(saveObj, () => {
      ...
      if (success) {
        ...
        adskipUtils.logDebug(
          `保存失败: ${chrome.runtime.lastError?.message || "未知错误"}`,
        );
      }
      resolve(success);
    });
...
async function verifyAdminAccess(apiKey) {
  ...
  // 简单的哈希检查
  const validKeyHash = "5a043dd";
  const inputHash = simpleHash(apiKey);
  const isValid = inputHash === validKeyHash;

  adskipUtils.logDebug(
    `管理员验证结果: ${isValid ? "通过" : "失败"}, 输入哈希: ${inputHash}, 有效哈希: ${validKeyHash}`,
  );
...
function setDebugMode(newValue) {
  return new Promise((resolve, reject) => {
    chrome.storage.local.set(
      { [STORAGE_KEYS.DEBUG_MODE]: newValue },
      function () {
        if (chrome.runtime.lastError) {
      ...
      if (result[STORAGE_KEYS.UPLOADER_WHITELIST]) {
        try {
          ...
          if (simpleHash !== lastWhitelistHash) {
            adskipUtils.logDebug("已加载UP主白名单", {
              data: whitelist,
              throttle: 5000,
            });
...
function saveUploaderWhitelist(whitelist) {
  return new Promise((resolve, reject) => {
    ...
    chrome.storage.local.set(
      { [STORAGE_KEYS.UPLOADER_WHITELIST]: JSON.stringify(formattedWhitelist) },
      () => {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
        } else {
          // 只在调试模式下输出详细白名单内容
          const logData = debugMode
            ? formattedWhitelist
            : { length: formattedWhitelist.length };
...
async function checkUploaderInWhitelist(uploaderName) {
  ...
  const match = whitelist.find(
    (item) => item.name === uploaderName && item.enabled !== false,
  );
...
async function addUploaderToWhitelist(uploader) {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      chrome.storage.local.set(
        { [STORAGE_KEYS.UPLOADER_WHITELIST]: JSON.stringify(whitelist) },
        function () {
          if (chrome.runtime.lastError) {
...
async function removeUploaderFromWhitelist(uploader) {
  ...
  try {
    ...
    if (newWhitelist.length < initialLength) {
      ...
      await new Promise((resolve, reject) => {
        chrome.storage.local.set(
          { [STORAGE_KEYS.UPLOADER_WHITELIST]: JSON.stringify(newWhitelist) },
          function () {
            if (chrome.runtime.lastError) {
              reject(new Error(chrome.runtime.lastError.message));
            } else {
...
async function importUploaderWhitelist(whitelistText) {
  ...
  uploaderNames.forEach((name) => {
    // 检查是否已存在
    const existingIndex = newWhitelist.findIndex((item) => item.name === name);
  ...
  return new Promise((resolve) => {
    try {
      ...
      for (let i = 0; i < uploaderSelectors.length; i++) {
        ...
        if (upElement) {
          adskipUtils.logDebug(
            `找到UP主元素，使用选择器：${uploaderSelectors[i]}`,
            { throttle: 10000 },
          );
          break; // 找到后立即停止搜索
        }
      ...
      adskipUtils.logDebug(`已更新UP主信息缓存: ${uploader} / ${title}`, {
        throttle: 5000,
      });
      resolve(info);
    } catch (e) {
...
async function toggleUploaderWhitelistStatus(uploaderName, enabled) {
  ...
  try {
    ...
    if (index >= 0) {
      ...
      adskipUtils.logDebug(
        `已${enabled ? "启用" : "禁用"}白名单UP主: ${uploaderName}`,
      );
    } else if (enabled) {
      // 如果不存在且需要启用，则添加
      whitelist.push({
...
function setSkipOwnVideos(enabled) {
  return new Promise((resolve, reject) => {
    chrome.storage.local.set(
      { [STORAGE_KEYS.SKIP_OWN_VIDEOS]: enabled },
      function () {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
          return;
...
function setSearchPrecheck(enabled) {
  return new Promise((resolve, reject) => {
    chrome.storage.local.set(
      { [STORAGE_KEYS.SEARCH_PRECHECK]: enabled },
      function () {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
          return;
...
function setReadMark(enabled) {
  return new Promise((resolve, reject) => {
    chrome.storage.local.set(
      { [STORAGE_KEYS.READ_MARK]: enabled },
      function () {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
          return;
      ...
      if (result[STORAGE_KEYS.VIDEO_WHITELIST]) {
        try {
          ...
          const sortedWhitelist = whitelist.sort((a, b) => {
            const timeA =
              typeof a === "string" ? 0 : a.addedAt || a.updatedAt || 0;
            const timeB =
              typeof b === "string" ? 0 : b.addedAt || b.updatedAt || 0;
            return timeB - timeA; // 倒序：新的在前
          });

          adskipUtils.logDebug("已加载视频白名单（按时间倒序）", {
            data: sortedWhitelist,
            throttle: 5000,
          });
        ...
        adskipUtils.logDebug("未找到视频白名单，返回空列表", {
          throttle: 5000,
        });
        resolve([]);
      }
...
function saveVideoWhitelist(whitelist) {
  return new Promise((resolve, reject) => {
    ...
    chrome.storage.local.set(
      { [STORAGE_KEYS.VIDEO_WHITELIST]: JSON.stringify(formattedWhitelist) },
      () => {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
        } else {
          adskipUtils.logDebug("已保存视频白名单", {
...
async function setSkipIntroEnabled(enabled) {
  return new Promise((resolve) => {
    chrome.storage.local.set(
      { [STORAGE_KEYS.SKIP_INTRO_ENABLED]: enabled },
      () => {
...
async function setSkipIntroDuration(duration) {
  ...
  return new Promise((resolve) => {
    chrome.storage.local.set(
      { [STORAGE_KEYS.SKIP_INTRO_DURATION]: value },
      () => {
...
async function setSkipOutroEnabled(enabled) {
  return new Promise((resolve) => {
    chrome.storage.local.set(
      { [STORAGE_KEYS.SKIP_OUTRO_ENABLED]: enabled },
      () => {
...
async function setSkipOutroDuration(duration) {
  ...
  return new Promise((resolve) => {
    chrome.storage.local.set(
      { [STORAGE_KEYS.SKIP_OUTRO_DURATION]: value },
      () => {
...
async function getSkipIntroOutroUploaderList() {
  return new Promise((resolve) => {
    chrome.storage.local.get(
      STORAGE_KEYS.SKIP_INTRO_OUTRO_UPLOADER_LIST,
      (result) => {
        if (result[STORAGE_KEYS.SKIP_INTRO_OUTRO_UPLOADER_LIST]) {
          try {
            resolve(
              JSON.parse(result[STORAGE_KEYS.SKIP_INTRO_OUTRO_UPLOADER_LIST]),
            );
          } catch (e) {
            resolve([]);
          }
...
async function saveSkipIntroOutroUploaderList(uploaderList) {
  return new Promise((resolve) => {
    chrome.storage.local.set(
      {
        [STORAGE_KEYS.SKIP_INTRO_OUTRO_UPLOADER_LIST]:
          JSON.stringify(uploaderList),
      },
      () => {
        adskipUtils.logDebug(
          `跳过开头/结尾UP主清单已保存，共 ${uploaderList.length} 个UP主`,
        );
        resolve(!chrome.runtime.lastError);
      },
    );
...
async function getLocalVideosProcessedCount() {
  ...
  return new Promise((resolve) => {
    chrome.storage.local.get(STORAGE_KEYS.LOCAL_VIDEOS_PROCESSED, (result) => {
      if (chrome.runtime.lastError) {
        adskipUtils.logDebug(
          `获取本地处理视频数量失败: ${chrome.runtime.lastError.message}`,
        );
        resolve(0);
        return;
      }
  ...
  return new Promise((resolve) => {
    chrome.storage.local.set(
      ...
      () => {
        if (chrome.runtime.lastError) {
          adskipUtils.logDebug(
            `更新本地处理视频数量失败: ${chrome.runtime.lastError.message}`,
          );
  ...
  return new Promise((resolve) => {
    chrome.storage.local.set(
      ...
      () => {
        if (chrome.runtime.lastError) {
          adskipUtils.logDebug(
            `保存用户统计信息缓存失败: ${chrome.runtime.lastError.message}`,
          );
          resolve(false);
          return;
        }
        ...
        adskipUtils.logDebug(
          `用户统计信息缓存已保存，更新时间: ${updateTimeDisplay}`,
        );
        resolve(true);
      },
    );
...
async function getUserStatsCache(maxAge = 3600000) {
  ...
  return new Promise((resolve) => {
    chrome.storage.local.get(STORAGE_KEYS.USER_STATS_CACHE, (result) => {
      if (chrome.runtime.lastError) {
        adskipUtils.logDebug(
          `获取用户统计信息缓存失败: ${chrome.runtime.lastError.message}`,
        );
        resolve(null);
        return;
      }
      ...
      if (now - cacheData.timestamp > maxAge) {
        adskipUtils.logDebug(
          `用户统计信息缓存已过期，过期时间：${(now - cacheData.timestamp) / 1000}秒`,
        );
      ...
      adskipUtils.logDebug(
        `成功获取用户统计信息缓存，缓存时间：${(now - cacheData.timestamp) / 1000}秒`,
      );
      resolve(cacheData);
    });
  });
  ...
  return new Promise((resolve) => {
    chrome.storage.local.set(
      ...
      () => {
        if (chrome.runtime.lastError) {
          adskipUtils.logDebug(
            `保存用户UID失败: ${chrome.runtime.lastError.message}`,
          );
          resolve(false);
          return;
        }
...
async function getUserUID() {
  ...
  return new Promise((resolve) => {
    chrome.storage.local.get(STORAGE_KEYS.USER_UID, (result) => {
      if (chrome.runtime.lastError) {
        adskipUtils.logDebug(
          `获取用户UID失败: ${chrome.runtime.lastError.message}`,
        );
        resolve(null);
        return;
      }
  ...
  return new Promise((resolve) => {
    chrome.storage.local.set(
      ...
      () => {
        if (chrome.runtime.lastError) {
          adskipUtils.logDebug(
            `保存用户名失败: ${chrome.runtime.lastError.message}`,
          );
          resolve(false);
          return;
        }
...
async function getUserUsername() {
  ...
  return new Promise((resolve) => {
    chrome.storage.local.get(STORAGE_KEYS.USER_USERNAME, (result) => {
      if (chrome.runtime.lastError) {
        adskipUtils.logDebug(
          `获取用户名失败: ${chrome.runtime.lastError.message}`,
        );
        resolve(null);
        return;
      }
    ...
    return new Promise((resolve) => {
      chrome.storage.local.set(
        ...
        () => {
          if (chrome.runtime.lastError) {
            adskipUtils.logDebug(
              `记录获取统计信息失败: ${chrome.runtime.lastError.message}`,
            );
            resolve(false);
            return;
          }
          ...
          adskipUtils.logDebug(
            `已记录本次获取用户统计信息，时间: ${new Date(now).toLocaleString()}, 视频数: ${videoCount}`,
          );
          resolve(true);
        },
      );
  ...
  return new Promise((resolve) => {
    chrome.storage.local.get(
      ...
      (result) => {
        if (chrome.runtime.lastError) {
          adskipUtils.logDebug(
            `获取上次统计获取信息失败: ${chrome.runtime.lastError.message}`,
          );
        ...
        adskipUtils.logDebug(
          `获取到上次统计获取信息: 时间: ${new Date(info.time).toLocaleString()}, 视频数: ${info.videoCount}`,
        );
        resolve(info);
      },
    );
...
async function shouldUpdateUserStats() {
  try {
    ...
    const videoCountCondition =
      currentVideoCount - lastFetchInfo.videoCount >= 2;
    ...
    return new Promise((resolve) => {
      ...
      chrome.storage.local.remove(
        ...
        () => {
          ...
          if (success) {
            adskipUtils.logDebug(
              "[AdSkip存储] 用户统计缓存已强制刷新，popup下次打开将重新获取最新数据",
            );
          } else {
            adskipUtils.logDebug(
              `[AdSkip存储] 强制刷新用户统计缓存失败: ${chrome.runtime.lastError?.message}`,
            );
          }
          resolve(success);
        },
...
async function saveQuotaExhaustedStatus(date) {
  try {
    return new Promise((resolve) => {
      chrome.storage.local.set(
        { [STORAGE_KEYS.QUOTA_EXHAUSTED_DATE]: date },
        () => {
          const success = !chrome.runtime.lastError;
          if (success) {
            adskipUtils.logDebug(
              `[AdSkip存储] 次数耗尽状态已保存，日期: ${date}`,
            );
          } else {
            adskipUtils.logDebug(
              `[AdSkip存储] 保存次数耗尽状态失败: ${chrome.runtime.lastError?.message}`,
            );
          }
          resolve(success);
        },
    ...
    return new Promise((resolve) => {
      chrome.storage.local.get(STORAGE_KEYS.QUOTA_EXHAUSTED_DATE, (result) => {
        if (
          ...
          !result[STORAGE_KEYS.QUOTA_EXHAUSTED_DATE]
        ) {
          resolve(null);
          return;
        }
    ...
    return new Promise((resolve) => {
      chrome.storage.local.remove(STORAGE_KEYS.QUOTA_EXHAUSTED_DATE, () => {
        ...
        if (success) {
          ...
          adskipUtils.logDebug(
            `[AdSkip存储] 清除次数耗尽状态失败: ${chrome.runtime.lastError?.message}`,
          );
        }
        resolve(success);
      });
...
async function checkAndClearQuotaIfNewDay() {
  try {
    ...
    if (quotaDate !== today) {
      ...
      adskipUtils.logDebug(
        `[AdSkip存储] 检测到新的一天 (${today})，已清除昨日的次数耗尽状态和失败视频缓存 (${quotaDate})`,
      );
      return true;
    }

    adskipUtils.logDebug(
      `[AdSkip存储] 仍在同一天 (${today})，次数耗尽状态保持`,
    );
    return false;
  } catch (e) {
        ...
        if (!failedVideos.includes(videoId)) {
          ...
          chrome.storage.local.set(
            ...
            () => {
              ...
              if (success) {
                adskipUtils.logDebug(
                  `[AdSkip存储] 视频 ${videoId} 已添加到次数耗尽失败缓存`,
                );
              } else {
                adskipUtils.logDebug(
                  `[AdSkip存储] 添加视频到失败缓存失败: ${chrome.runtime.lastError?.message}`,
                );
              }
              resolve(success);
            },
  ...
  try {
    ...
    return new Promise((resolve) => {
      chrome.storage.local.get(STORAGE_KEYS.QUOTA_FAILED_VIDEOS, (result) => {
        if (
          chrome.runtime.lastError ||
          !result[STORAGE_KEYS.QUOTA_FAILED_VIDEOS]
        ) {
          resolve(false);
          return;
        }
        try {
          ...
          adskipUtils.logDebug(
            `[AdSkip存储] 检查视频 ${videoId} 是否在失败缓存中: ${isInCache}`,
          );
          resolve(isInCache);
        } catch (e) {
    ...
    return new Promise((resolve) => {
      chrome.storage.local.remove(STORAGE_KEYS.QUOTA_FAILED_VIDEOS, () => {
        ...
        if (success) {
          ...
          adskipUtils.logDebug(
            `[AdSkip存储] 清除失败视频缓存失败: ${chrome.runtime.lastError?.message}`,
          );
        }
        resolve(success);
      });
    ...
    chrome.storage.local.get(STORAGE_KEYS.POPUP_OPEN_COUNT, (result) => {
      ...
      chrome.storage.local.set(
        ...
        () => {
          if (chrome.runtime.lastError) {
            adskipUtils.logDebug(
              `[AdSkip存储] 增加popup打开计数失败: ${chrome.runtime.lastError.message}`,
            );
    ...
    chrome.storage.local.get(STORAGE_KEYS.SHARE_CLICK_COUNT, (result) => {
      ...
      chrome.storage.local.set(
        ...
        () => {
          if (chrome.runtime.lastError) {
            adskipUtils.logDebug(
              `[AdSkip存储] 增加分享按钮点击计数失败: ${chrome.runtime.lastError.message}`,
            );
            resolve(currentCount);
            return;
          }
...
async function getUsageStats() {
  ...
  return new Promise((resolve) => {
    chrome.storage.local.get(
      [STORAGE_KEYS.POPUP_OPEN_COUNT, STORAGE_KEYS.SHARE_CLICK_COUNT],
      (result) => {
        if (chrome.runtime.lastError) {
          adskipUtils.logDebug(
            `[AdSkip存储] 获取使用统计数据失败: ${chrome.runtime.lastError.message}`,
          );
          resolve({
            popupOpens: 0,
            shareClicks: 0,
...
async function getSubtitleTimelineDefaultCollapsed() {
  return new Promise((resolve) => {
    chrome.storage.local.get(
      STORAGE_KEYS.SUBTITLE_TIMELINE_DEFAULT_COLLAPSED,
      (result) => {
        if (chrome.runtime.lastError) {
          resolve(false);
          return;
        ...
        resolve(
          result[STORAGE_KEYS.SUBTITLE_TIMELINE_DEFAULT_COLLAPSED] === true,
        );
      },
    );
  });
...
async function getAiChatTrialUsedCount() {
  return new Promise((resolve) => {
    chrome.storage.local.get(
      STORAGE_KEYS.AI_CHAT_TRIAL_USED_COUNT,
      (result) => {
        if (chrome.runtime.lastError) {
          resolve(0);
          return;
...
async function getProUpdateAdsLastDate() {
  return new Promise((resolve) => {
    chrome.storage.local.get(
      STORAGE_KEYS.PRO_UPDATE_ADS_LAST_DATE,
      (result) => {
        if (chrome.runtime.lastError) {
          resolve("");
          return;
...
function parseIsProFromStats(data) {
  const tier = normalizeAccountTypeString(data?.tier);
  return tier === "pro";
}
...
async function getFeatureEntitlement(forceRefresh = false) {
  ...
  const cached = await new Promise((resolve) => {
    chrome.storage.local.get(
      STORAGE_KEYS.FEATURE_ENTITLEMENT_CACHE,
      (result) => {
        if (chrome.runtime.lastError) {
          resolve(null);
          return;
...
function resolveQuotaAvailability(rawEntitlement, quotaKey) {
  const quota = rawEntitlement?.quotas?.[quotaKey];
  ...
  const dailyLimit = Math.max(0, Number(quota.daily_limit || 0));
  const dailyUsed = Math.max(0, Number(quota.daily_used || 0));
  const bonusPoolRemain = Math.max(0, Number(quota.bonus_pool_remain || 0));
  const remaining = Math.max(0, dailyLimit + bonusPoolRemain - dailyUsed);
  ...
  return {
    ...
    dailyUsed,
    bonusPoolRemain,
    message: remaining > 0 ? "" : "今日可用次数已用完",
  };
}

async function getQuotaEntitlement(quotaKey, forceRefresh = false) {
  const entitlement = await getFeatureEntitlement(forceRefresh);
  const quotaInfo = resolveQuotaAvailability(entitlement?.raw || {}, quotaKey);
  return {
    ...quotaInfo,
    raw: entitlement?.raw || {},
  };
 ...
 * @returns {Promise<Object>} 后端返回的兑换结果
 */
async function redeemActivationCode(code) {
  ...
  if (!uid) {
    uid = (await getUserUID()) || 0;
  }

  if (!uid) {
    return { success: false, message: "请先登录B站账号后再兑换激活码" };
  }

  const apiUrls = await getApiUrls();
  const response = await fetch(apiUrls.redeem, {
    method: "POST",
  ...
  if (!response.ok) {
    ...
    return {
      success: false,
      message: errorData.message || `兑换失败 (${response.status})`,
    };
  }
  ...
  // 兑换成功后立即刷新本地权益缓存
  if (result.success) {
    await forceRefreshUserStatsCache().catch(() => {});
    await refreshFeatureEntitlement().catch(() => {});
    adskipUtils.logDebug(
      "[AdSkip存储] 激活码兑换成功，已刷新权益缓存和统计缓存",
    );
  }
...
/**
 * 获取单项配额的极简展示文本（无斜杠、平滑降级）
 * @param {string} quotaKey - 配额类型 (ai_chat / video_analyze / ad_correction / audio_subtitle)
 * @param {Object} rawEntitlement - 可选，已有的完整权益原始数据，传入可避免重复请求
 * @returns {{ text: string, status: 'normal'|'warn'|'exhausted', remaining: number }}
 */
function getConsolidatedQuotaText(quotaKey, rawEntitlement) {
  const quota = rawEntitlement?.quotas?.[quotaKey];
  if (!quota || typeof quota !== "object") {
    return { text: "暂无数据", status: "exhausted", remaining: 0 };
  }

  const dailyLimit = Math.max(0, Number(quota.daily_limit || 0));
  const dailyUsed = Math.max(0, Number(quota.daily_used || 0));
  const bonusPoolRemain = Math.max(0, Number(quota.bonus_pool_remain || 0));
  const dailyRemain = Math.max(0, dailyLimit - dailyUsed);

  // 平滑降级：日配额优先 → 体验次数备用 → 耗尽
  if (dailyRemain > 0) {
    return {
      text: `今日可用 ${dailyRemain}/${dailyLimit}`,
      status: "normal",
      remaining: dailyRemain + bonusPoolRemain,
    };
  }
  if (bonusPoolRemain > 0) {
    return {
      text: `剩余体验次数 ${bonusPoolRemain}`,
      status: "warn",
      remaining: bonusPoolRemain,
    };
  }
  return { text: "0 次可用", status: "exhausted", remaining: 0 };
}
...
async function cacheExternalConfig(data) {
  try {
    ...
    await new Promise((resolve) => {
      chrome.storage.local.set(
        { [EXTERNAL_CONFIG_CACHE_KEY]: cacheData },
        resolve,
      );
    });
...
async function loadExternalConfig() {
  ...
  if (PRIME_BASE_URL) {
    console.log(
      "[AdSkip存储] 检测到PRIME_BASE_URL设置，API请求将使用开发环境地址:",
      PRIME_BASE_URL,
    );
  }
  ...
  try {
    ...
    if (!config) {
      ...
      try {
        ...
        await new Promise((resolve) => {
          chrome.storage.local.set(
            { [EXTERNAL_CONFIG_CACHE_KEY]: shortCacheData },
            resolve,
          );
        });
      }
...
function buildApiUrls(config) {
  ...
  if (PRIME_BASE_URL) {
    ...
    baseURL =
      config?.api?.adSkipServerBaseURL ||
      DEFAULT_CONFIG.api.adSkipServerBaseURL;
  ...
  return {
    ...
    redeem: `${baseURL}/api/user/redeem`,
  ...
  try {
    ...
    if (serverData.local_popup_opens !== undefined) {
      ...
      if (serverPopupOpens > localPopupOpens) {
        ...
        hasUpdates = true;
        adskipUtils.logDebug(
          `[AdSkip存储] 更新popup打开次数: ${localPopupOpens} -> ${serverPopupOpens}`,
        );
      }
    }
    ...
    if (serverData.local_share_clicks !== undefined) {
      ...
      if (serverShareClicks > localShareClicks) {
        ...
        hasUpdates = true;
        adskipUtils.logDebug(
          `[AdSkip存储] 更新分享点击次数: ${localShareClicks} -> ${serverShareClicks}`,
        );
      }
    }
      ...
      return new Promise((resolve) => {
        chrome.storage.local.set(updates, () => {
          ...
          if (success) {
            ...
            adskipUtils.logDebug(
              `[AdSkip存储] 服务端数据同步失败: ${chrome.runtime.lastError?.message}`,
            );
          }
          resolve(success);
        });
...
/**
 * 持久化保存强制转录的字幕
 * @param {string} videoId 视频ID（如BV号）
 * @param {Array} subtitles 听译出来的字幕数组
 */
async function saveForcedTranscribedSubtitles(videoId, subtitles) {
  if (!videoId || !subtitles) return;
  const key = `${STORAGE_KEYS.TRANSCRIBED_SUBTITLE_PREFIX}${videoId}`;
  try {
    await new Promise((resolve) => {
      chrome.storage.local.set({ [key]: subtitles }, resolve);
    });
    adskipUtils.logDebug(
      `[AdSkip存储] 已持久化保存 ${subtitles.length} 条强制转录字幕: ${key}`,
    );
  } catch (e) {
    adskipUtils.logDebug(`[AdSkip存储] 保存强转字幕失败: ${e.message}`);
  }
}
 ...
 */
async function loadForcedTranscribedSubtitles(videoId) {
  ...
  try {
    ...
    adskipUtils.logDebug(`[AdSkip存储] 加载强转字幕失败: ${e.message}`);
    return null;
  }
}
 ...
 * 获取播放超过15秒的已读标记
 */
async function getMarkedReadBVs() {
  try {
    const result = await new Promise((resolve) => {
      chrome.storage.local.get([STORAGE_KEYS.READ_BVS], resolve);
    });
    return result[STORAGE_KEYS.READ_BVS] || [];
  } catch (e) {
    return [];
  }
}
...
/**
 * 标记视频为已读（保存其BV，去重，最多500条）
 * @param {string} videoId BV号
 */
async function markVideoAsRead(videoId) {
  if (!videoId || typeof videoId !== "string") return;
  const cacheKey = videoId.replace("adskip_", ""); // 取纯BV
  if (!cacheKey.startsWith("BV")) return;

  try {
    const bvs = await getMarkedReadBVs();
    if (!bvs.includes(cacheKey)) {
      bvs.push(cacheKey);
      // 限制最多只存1000条兜底记录
      if (bvs.length > 1000) {
        bvs.shift();
      ...
      await new Promise((resolve) => {
        chrome.storage.local.set({ [STORAGE_KEYS.READ_BVS]: bvs }, resolve);
      });
      adskipUtils.logDebug(`[AdSkip存储] 新增兜底已读标记: ${cacheKey}`);
    }
  } catch (e) {
    adskipUtils.logDebug(`[AdSkip存储] 标记已读失败: ${e.message}`);
  }
}
...
window.adskipStorage = {
  ...
  getMarkedReadBVs,
  markVideoAsRead,
  saveForcedTranscribedSubtitles,
  loadForcedTranscribedSubtitles,
  getApiUrls,
  buildApiUrls,
  ...
  try {
    ...
    const customConfig = await new Promise((resolve) => {
      chrome.storage.sync.get(
        ["customServerEnabled", "customServerUrl"],
        resolve,
      );
    });
    ...
    if (
      externalConfig &&
      externalConfig.api &&
      externalConfig.api.adSkipServerBaseURL
    ) {
      adskipUtils.logDebug(
        "使用外部配置服务器:",
        externalConfig.api.adSkipServerBaseURL,
