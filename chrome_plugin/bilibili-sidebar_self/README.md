...
展示哪些信息说明价值又不会觉得监视？
还需要一个捐赠跳转
外部链接打开的时候无法请求数据吗？肯定是可以的，缓存uid就行

1.有用过10次以上才显示赞赏，就是文字和二维码
2.准备发布的gemini api和体验逻辑
3.个人主页
4.数据库改ORM是后端后续的事
大会员B币发电
...
1.版本号统一 done
2.调整管理员入口  done
3.开源插件？反正发布也是开源的？还是混淆一下？——
4.最后调整一下预览
5.检查一些设置和gemini的apikey的频率限制
6.远端服务器同步之后再做客户端的验证
7.花一小时思考账号的数据策略
发布之后再考虑整体review和重构
识别年度大会员，给一些额外的免费次数？——已经识别到了，这个数据看看是不是给后端。
