...
async function init() {
  ...
  adskipUtils.logDebug(
    `初始化 - 管理员状态: ${isAdminAuthorized ? "已授权" : "未授权"}`,
  );
  ...
  // 无条件启动播放时间监控（用于15秒兜底已读标记等，不依赖广告/字幕状态）
  if (typeof adskipVideoMonitor.setupPlaybackTimeMonitor === "function") {
    adskipVideoMonitor.setupPlaybackTimeMonitor();
  }
    ...
    if (namespace === "local") {
      ...
      if (changes[adskipStorage.KEYS.PERCENTAGE]) {
        ...
        if (adSkipPercentage !== newPercentage) {
          ...
          adskipUtils.logDebug(
            `检测到广告跳过百分比设置变化: ${adSkipPercentage}%`,
          );
      ...
      if (changes[adskipStorage.KEYS.ADMIN_AUTH]) {
        const newAdminStatus =
          changes[adskipStorage.KEYS.ADMIN_AUTH].newValue === true;
        if (isAdminAuthorized !== newAdminStatus) {
          isAdminAuthorized = newAdminStatus;
          adskipUtils.logDebug(
            `检测到管理员权限变化: ${isAdminAuthorized ? "已授权" : "未授权"}`,
          );
        }
      }
      ...
      if (changes[adskipStorage.KEYS.DEBUG_MODE] !== undefined) {
        ...
        if (currentDebugMode !== newDebugMode) {
          adskipStorage.setDebugMode(newDebugMode).then(() => {
            adskipUtils.logDebug(
              `检测到调试模式变化: ${newDebugMode ? "已启用" : "已禁用"}`,
            );
          });
        }
      }
...
async function applyNewAdTimestamps(newTimestamps) {
  ...
  if (
    ...
    adskipUtils.logDebug(
      "[AdSkip Core] 未找到adskipVideoMonitor或setupAdSkipMonitor",
    );
  }
  ...
  try {
    ...
    adskipUtils.logDebug(
      `[AdSkip Core] 已将时间戳保存到视频${videoId}的存储中`,
    );
  } catch (error) {
    adskipUtils.logDebug(
      `[AdSkip Core] 保存时间戳时发生错误: ${error.message}`,
    );
  }
}
