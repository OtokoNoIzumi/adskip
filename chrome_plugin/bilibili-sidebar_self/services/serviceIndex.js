...
(function () {
  ...
  async function getBilibiliLoginStatus() {
    try {
      ...
      if (
        loginStatusCache &&
        now - loginStatusCacheTimestamp < LOGIN_CACHE_LIFETIME
      ) {
...
(function () {
  ...
  async function getVideoSubtitles() {
    try {
      ...
      if (
        subtitlesCache &&
        now - subtitlesCacheTimestamp < CACHE_LIFETIME &&
        subtitlesCache.rawData.bvid === bvid
      ) {
        adskipUtils.logDebug("[AdSkip服务] 使用缓存的字幕列表数据", {
          data: subtitlesCache,
          timestamp: subtitlesCacheTimestamp,
        });
  ...
  async function getVideoData(forceRefresh = false) {
    ...
    if (
      !forceRefresh &&
      videoDataCache &&
      now - cacheTimestamp < CACHE_LIFETIME
    ) {
    ...
    try {
        ...
        try {
          ...
          if (data.code === 0 && data.result) {
            ...
            if (epInfo) {
              ...
              result.title =
                epInfo.long_title || epInfo.ep_title || epInfo.title;
      ...
      adskipUtils.logDebug(
        "[AdSkip服务] 最终获取到的视频元数据" +
          (hasCompleteInfo ? "完整" : "不完整") +
          ":",
        {
          bvid: result.bvid,
          aid: result.aid,
  ...
  async function downloadSubtitleFile(url) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `下载字幕失败: ${response.status} ${response.statusText}`,
        );
      }
      ...
      if (
        rawSubtitleData &&
        rawSubtitleData.body &&
        Array.isArray(rawSubtitleData.body)
      ) {
        // 保存原始数据引用（可以用于特殊场景）
        ...
        // 生成预览内容（前10条，格式化时间）
        processedData.preview = rawSubtitleData.body
          .slice(0, 10)
          .map((item) => ({
  ...
  async function getSubtitlePreview() {
    try {
      ...
      adskipUtils.logDebug(
        "[AdSkip服务] 获取到字幕列表:",
        subtitleInfo.subtitles.length,
        "个字幕",
      );

      // 提取可用语言列表
      result.availableLanguages = subtitleInfo.subtitles.map(
        (sub) => sub.languageName,
      );
      adskipUtils.logDebug(
        "[AdSkip服务] 可用字幕语言:",
      ...
      for (let sub of subtitleInfo.subtitles) {
        if (
          sub.language &&
          (sub.language.includes("zh") || sub.language.includes("cn"))
        ) {
      ...
      if (!selectedSubtitle) {
        ...
        if (selectedSubtitle && subtitleInfo.rawData) {
          ...
          const title =
            subtitleInfo.rawData.rawVideoData?.title ||
            subtitleInfo.rawData.title ||
            "未知";
          adskipUtils.logDebug(
            "[AdSkip服务] 非中文字幕：bvid=" +
              bvid +
      ...
      if (selectedSubtitle && selectedSubtitle.url) {
        ...
        try {
          // 下载字幕文件 - 已处理格式
          const processedSubtitle = await downloadSubtitleFile(
            selectedSubtitle.url,
          );
          ...
          adskipUtils.logDebug(
            "[AdSkip服务] 提取到",
            result.subtitleContent.length,
            "条字幕预览，共",
            processedSubtitle.subtitles.length,
            "条完整字幕",
          );
        } catch (downloadError) {
