    ...
    if (age > CACHE_TTL) {
      ...
      await new Promise((resolve) => {
        chrome.storage.local.set(
          { [CACHE_KEY]: JSON.stringify(cache) },
          resolve,
        );
      });
      return null;
...
async function checkSubtitle(bv) {
  ...
  if (cached !== null) {
    console.log(
      `[SearchPrecheck] 使用缓存结果: ${bv} -> ${cached.hasSubtitle}`,
    );
...
async function runTask(bv, taskId) {
  try {
    ...
    if (viewData.code !== 0)
      throw new Error(`API ${viewData.code}: ${viewData.message}`);
    ...
    if (!subtitleResponse.ok)
      throw new Error(`subtitle HTTP ${subtitleResponse.status}`);
    const subtitleData = await subtitleResponse.json();
    if (subtitleData.code !== 0)
      throw new Error(`subtitle ${subtitleData.code}: ${subtitleData.message}`);

    const subtitles = subtitleData?.data?.subtitle?.subtitles || [];
    const hasSubtitle = subtitles.length > 0;
    console.log(
      `[SearchPrecheck] ${bv}: ${hasSubtitle ? "有字幕" : "无字幕"} (${title.substring(0, 20)}...)`,
    );
...
function addNoSubtitleBadge(card) {
  ...
  if (
    card.querySelector(".adskip-ad-label") ||
    card.querySelector(".adskip-read-badge") ||
    card.querySelector(".adskip-no-subtitle-badge")
  ) {
    return;
  }
...
function addReadBadge(card) {
  ...
  if (
    card.querySelector(".adskip-ad-label") ||
    card.querySelector(".adskip-read-badge") ||
    card.querySelector(".adskip-read-label")
  ) {
    return;
  }
  ...
  const observer = new MutationObserver((mutations) => {
      ...
      if (mutation.type === "childList" && mutation.addedNodes.length) {
        for (const node of mutation.addedNodes) {
          if (node.nodeType === Node.ELEMENT_NODE) {
            const anchors =
              node.querySelectorAll?.('a[href*="/video/BV"]') || [];
    ...
    if (newAnchors.size > 0) {
      ...
      if (adCount > 0 || readCount > 0) {
        console.log(
          `[SearchPrecheck] 监听到新增锚点，广告: ${adCount}，已读: ${readCount}，待检测: ${uniqueBV.size}`,
        );
      } else {
        console.log(
          `[SearchPrecheck] 监听到新增锚点，共 ${newAnchors.size}，唯一BV ${uniqueBV.size}`,
        );
      }
    }
  });
...
async function processExistingCards() {
  ...
  if (adCount > 0 || readCount > 0) {
    console.log(
      `[SearchPrecheck] 开始处理页面上 ${anchors.length} 个锚点，广告: ${adCount}，已读: ${readCount}，待检测: ${uniqueBV.size}`,
    );
  } else {
    console.log(
      `[SearchPrecheck] 开始处理页面上 ${anchors.length} 个锚点，唯一BV ${uniqueBV.size}`,
    );
  }
...
async function loadReadBVs() {
  try {
    ...
    // 3. 加载兜底播放(>15s)的已读标记
    if (typeof window.adskipStorage.getMarkedReadBVs === "function") {
      const markedReads = await window.adskipStorage.getMarkedReadBVs();
      markedReads.forEach((bv) => {
        if (bv) newReadBVs.add(bv);
      });
    }

    readBVs = newReadBVs;
    console.log(
      `[SearchPrecheck] 已加载 ${readBVs.size} 个已读BV（白名单: ${whitelist.length}, 广告数据: ${videoDataKeys.length}）`,
    );
  } catch (error) {
...
async function loadReadMarkSetting() {
  try {
    ...
    console.log(
      `[AdSkip] 已读标记开关状态: ${readMarkEnabled ? "开启" : "关闭"}`,
    );
...
async function initAdMarking() {
  ...
  if (adCount > 0 || readCount > 0) {
    console.log(
      `[AdSkip] 标记了 ${adCount} 个广告卡片，${readCount} 个已读卡片`,
    );
  }
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    if (newAdCount > 0 || newReadCount > 0) {
      console.log(
        `[AdSkip] 新标记了 ${newAdCount} 个广告卡片，${newReadCount} 个已读卡片`,
      );
    }
  });
...
if (window.location.hostname === "search.bilibili.com") {
  ...
  chrome.storage.local.onChanged.addListener((changes) => {
    if (changes.adskip_search_precheck) {
      ...
      changes.adskip_video_whitelist ||
      changes.adskip_read_bvs ||
      Object.keys(changes).some((key) => key.startsWith("adskip_BV"))
    ) {
      // 白名单、广告数据或15秒兜底标记变化时重新加载已读BV
      loadReadBVs().then(() => {
        // 重新处理当前页面卡片
        processExistingCards();
      });
