...
function isLogFiltered(message) {
  ...
  if (logFilterMap.size > 50) {
    const oldEntries = [...logFilterMap.entries()].filter(
      ([_, time]) => now - time > 10000,
    ); // 清理超过10秒的记录
...
function logDebug(message, dataOrOptions, throttleTime = 0) {
  ...
  if (
    ...
    dataOrOptions.hasOwnProperty("throttle")
  ) {
    // 如果是配置对象格式
  ...
  if (throttle > 0) {
    ...
    if (logThrottleMap.size > 100) {
      const oldEntries = [...logThrottleMap.entries()].filter(
        ([_, time]) => now - time > 60000,
      ); // 清理超过1分钟的记录
...
function formatTimestampsForDisplay(timestamps) {
  ...
  return timestamps
    .map((ad) => formatSingleTimestamp(ad.start_time, ad.end_time))
    .join(", ");
}
...
function isOverlapping(segment1, segment2) {
  return (
    segment1.start_time <= segment2.end_time &&
